...
*/ //commented by ac.dilag 5/25/15
    ...
    var myjstblDbOccurance;
    var myjstblWebOccurance;
    var myjstblSyncOccurance;
    var tabOccurance = document.createElement('table');
    tabOccurance.id="tableid4";

    var tabWebOccurance = document.createElement('table');
    tabWebOccurance.id="tableid5";
    var tabSyncOccurance = document.createElement('table');
    tabSyncOccurance.id="tableid5";
    var colarrayOccurance = [];
    var parametersOccurance = [];
    var colarrayparamsOccurance=["Versions","Count","Percentage"];
    for (var x=0; x<3; x++)
    {
        parametersOccurance[x] = document.createElement('span');
        colarrayOccurance[colarrayparamsOccurance[x]] = { 
            header_title: colarrayparamsOccurance[x],
            edit: [parametersOccurance[x]],
            disp: [parametersOccurance[x]],
            td_class: "tablerow tdall "+"td"+colarrayparamsOccurance[x],
            headertd_class : "tdall"
        };
    }
    ...
    $(function(){
        ...
        $("#filtervesiondb").chosen({allow_single_deselect:true, 
            no_results_text: "No",
            add_item_enable: false}); //ac.dilag

        $("#filtervesionweb").chosen({allow_single_deselect:true, 
            no_results_text: "No",
            add_item_enable: false}); //ac.dilag

        $("#filtervesionsync").chosen({allow_single_deselect:true, 
            no_results_text: "No",
            add_item_enable: false}); //ac.dilag
    ...
    function load_tables()
    {
        ...
        myjstblDbOccurance = new my_table(tabOccurance, colarrayOccurance , {ispaging : false,
                                                tdhighlight_when_hover : "tablerow3",
                                                iscursorchange_when_hover : true});
        var rootDbOccurance = document.getElementById("tbldbperc");
        rootDbOccurance.appendChild(myjstblDbOccurance.tab);

        myjstblWebOccurance = new my_table(tabWebOccurance, colarrayOccurance , {ispaging : false,
                                                tdhighlight_when_hover : "tablerow3",
                                                iscursorchange_when_hover : true});
        var rootWebOccurance = document.getElementById("tblwebperc");
        rootWebOccurance.appendChild(myjstblWebOccurance.tab);
    

        myjstblSyncOccurance = new my_table(tabSyncOccurance, colarrayOccurance , {ispaging : false,
                                                tdhighlight_when_hover : "tablerow3",
                                                iscursorchange_when_hover : true});
        var rootSyncOccurance = document.getElementById("tblautosyncperc");
        rootSyncOccurance.appendChild(myjstblSyncOccurance.tab);    
    ...
    {  
        ...
        $.getJSON("<?=base_url()?>client/search_control",
        {
            ...
            db_mdl : $('#filtervesiondb').val(),
            sync_mdl : $('#filtervesionsync').val(),
            web_mdl: $('#filtervesionweb').val(),         
    ...
    function get_Version_Instances()
    {  
        myjstblDbOccurance.clean_table();
        //myjstblWebOccurance.clean_table();
        //myjstblSyncOccurance.clean_table();
       
        $.getJSON("<?=base_url()?>client/select_NbofOccurance_control",
        {
            fieldName_mdl : "database"
        }
        ,
        function(data){
            myjstblDbOccurance.insert_multiplerow_with_value(1,data);
        });  

        $.getJSON("<?=base_url()?>client/select_NbofOccurance_control",
        {
            fieldName_mdl : "websystem"
        }
        ,
        function(data){
            myjstblWebOccurance.insert_multiplerow_with_value(1,data);
        });  


        $.getJSON("<?=base_url()?>client/select_NbofOccurance_control",
        {
            fieldName_mdl : "autosync"
        }
        ,
        function(data){
            myjstblSyncOccurance.insert_multiplerow_with_value(1,data);
        });  
    }
