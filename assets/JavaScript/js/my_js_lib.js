...
function my_autocomplete_add(sel, url_ac, options ){
	...
	$(sel).live("focus", function (event) {
		$(this).autocomplete({
			...
			delay: 0,
			source: function(req, add){
				$.getJSON(url_ac, req, function(data) {
					var suggestions = [];
...
function fire_tab_when_entered_table(tab){
	$($("#"+tab).find('input')).not( $(":button") ).unbind("keypress");
	$($("#"+tab).find('input')).not( $(":button") ).keypress( function (evt) {
		if (evt.keyCode == 13) {
			iname = $(this).val();
			if (iname !== 'Submit'){
				next_focus(this);
	...
	$($("#"+tab).find('select')).not( $(":button") ).keypress( function (evt) {
		if (evt.keyCode == 13) {
			iname = $(this).val();
			if (iname !== 'Submit'){
				next_focus(this);
...
function next_focus(elem) {
	...
	if ( index > -1 && ( index + 1 ) < fields.length ) {
		index++;
		while( $(fields.eq( index )).parent().css("display") == "none" || 
				$(fields.eq( index )).parent().parent().css("display") == "none" ||
				$(fields.eq( index )).attr("disabled") == "disabled" ||
				$(fields.eq( index )).attr("readonly") == "readonly" ||
				$(fields.eq( index )).attr("type") == "hidden" || 
				$(fields.eq( index )).css("display") == "none" )
		{
			index++;
		}
...
function NT_KeyPressEvent(Control, KeyValue, FuncCall){
    $(Control).keypress(function(e){
        if(e.keyCode == KeyValue){
            FuncCall(Control.val());
            e.preventDefault();
        }
    }
)}
